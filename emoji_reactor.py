...
try:
    ...
    hands_up_emoji = cv2.imread("despair.jpg")
    ...
    if hands_up_emoji is None:
        raise FileNotFoundError("despair.jpg not found")
    ...
    print("- despair.jpg (hands up)")
    exit()

blank_emoji = np.zeros((WINDOW_HEIGHT, WINDOW_WIDTH, 3), dtype=np.uint8)
# NumPy expects height, width, channels, while openCV resize expects width, height

# Start webcam
# CAP_DSHOW avoids the slow MSMF backend probing / warning spam on Windows
